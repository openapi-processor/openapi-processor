...
[NOTE]
====
By default, openapi-processor does not run the code formatter.
====

Running openapi-processor with link:{code-formatter}[java code formatter] on JDK 16 or newer requires a bit of extra configuration.

Unfortunately it depends on internal jdk packages that are link:{jep396}[strongly encapsulated] since JDK 16. It is necessary to export a few packages.

Alternatively, it is possible to use the Eclipse code formatter that does not need any extra configuration. See
xref:spring::processor/configuration.adoc#format-code[formatting options].
