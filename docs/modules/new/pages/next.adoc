...
the processor automatically generates names based on the yaml/json path for inline types, (so they are unique). Maybe nice to map the generated name to a nicer name.
...
- native compilation, it is possible, don't know yet how to run native test, compiling tests as a native image fails.
