...
The newest openapi-processor-spring/micronaut 2025.6 releases add support for OpenAPI 3.2.
...
+
It is used here with the link:package-names-from-location.adoc[_package-name-from-location_] feature and simplifies the `$ref` s in the OpenAPI description.
...
+
It is used to show two sample streaming apis using Spring `SseEmitter` and `StreamingResponseBody`.
